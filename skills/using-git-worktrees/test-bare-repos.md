...
**Existing Worktrees:**
- `.worktrees/main/` (primary workspace)
- `.worktrees/feature-a/` (example existing feature)
...
# /path/to/bare-repo/.worktrees/feature-a [feature-a]
...
# Should show: main/, feature-a/, feature-b/
...
### Variation 1: Operations in Primary Workspace
```bash
cd .worktrees/main
# Agent should understand this is the "main" branch equivalent
# Should be able to pull, commit, etc. here
```

### Variation 2: Branching from Existing Worktree
```bash
# Create feature-c based on feature-a
cd /path/to/bare-repo
git worktree add .worktrees/feature-c -b feature-c feature-a
# Should understand branching in bare repo context
```

### Variation 3: Cleanup in Bare Repo
```bash
# Removing worktree from bare repo
git worktree remove .worktrees/feature-b
git branch -D feature-b
# Should understand cleanup in bare repo context
```
...
# Remove branches
git branch -D feature-b feature-a main
...
This test addresses real production confusion where agent attempted `git pull` in bare repository root (fails) and was unclear about primary workspace location in bare repository setup.

## Why Bare Repositories Matter

Bare repositories are common in certain workflows:
- Artifact-bridge uses bare repos with `.worktrees/main/` as primary workspace
