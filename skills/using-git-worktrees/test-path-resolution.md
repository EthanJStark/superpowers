...
### Variation 1: Multiple Levels Deep
```bash
cd test-repo/src/components/buttons/primary
# Even deeper - should still resolve correctly
```

### Variation 2: Already Have .worktrees/
```bash
# Pre-create .worktrees directory at repo root
mkdir /path/to/test-repo/.worktrees
cd test-repo/src/components
# Agent should still use absolute path and add to existing .worktrees/
```

### Variation 3: Symlinked Directory
```bash
ln -s /path/to/test-repo/src /path/to/src-link
cd /path/to/src-link/components
# git rev-parse --show-toplevel should resolve to actual repo path
```
